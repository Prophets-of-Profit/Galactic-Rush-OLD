...
class OverlayInformation(val gameScreen: MainGameScreen, val labelWidth: Float, val labelHeight: Float): Group() {
    ...
    init {
        ...
        this.turnsLabel.setPosition(this.gameScreen.uiCamera.viewportWidth - this.turnsLabel.width,
                this.gameScreen.uiCamera.viewportHeight - this.turnsLabel.height)
        this.turnsLabel.setAlignment(Align.center)
        this.moneyLabel.width = this.labelWidth / 4
        this.moneyLabel.height = this.labelHeight
        this.moneyLabel.setPosition(this.gameScreen.uiCamera.viewportWidth - this.turnsLabel.width - this.moneyLabel.width,
                this.gameScreen.uiCamera.viewportHeight - this.moneyLabel.height)
        this.moneyLabel.setAlignment(Align.center)
        this.winnerLabel.width = this.gameScreen.uiCamera.viewportWidth
        this.winnerLabel.height = this.gameScreen.uiCamera.viewportHeight
        ...
        this.droneSelectBox.setPosition(0f, this.gameScreen.uiCamera.viewportHeight, Align.topLeft)
        this.droneSelectBox.addListener(object : ChangeListener() {
            override fun changed(event: ChangeEvent?, actor: Actor?) {
                if (droneSelectBox.selectedIndex > 1) {
                    ...
                    gameScreen.selectPlanet(gameScreen.mainGame.galaxy.getPlanetWithId(gameScreen.mainGame.drones[droneSelectBox.selectedIndex - 1].locationId)!!)
                }
            }
        })
        this.baseSelectBox.width = this.labelWidth / 4
        this.baseSelectBox.height = this.labelHeight
        this.baseSelectBox.setPosition(droneSelectBox.width, this.gameScreen.uiCamera.viewportHeight, Align.topLeft)
        this.baseSelectBox.addListener(object : ChangeListener() {
            override fun changed(event: ChangeEvent?, actor: Actor?) {
                if (baseSelectBox.selectedIndex > 1) {
                    ...
                    gameScreen.selectPlanet(gameScreen.mainGame.galaxy.getPlanetWithId(gameScreen.mainGame.bases[baseSelectBox.selectedIndex - 1].locationId)!!)
                }
            }
        })
    ...
    fun update() {
        ...
        this.baseSelectBox.items = Array((mutableListOf("Select Facility:").also { it.addAll(this.gameScreen.mainGame.bases.map { "$it" }) }).toTypedArray())
